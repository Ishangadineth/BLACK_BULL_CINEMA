        ...
        if (data.startsWith("view_")) {
          ...
          if (searchKey) {
            ...
            if (existingStr) {
              ...
              const qualText = T.qual_sel || "හරි, දැන් ඔයා කැමතිම කොලිටි එක තෝරගන්නෝ... 😉👇";
        ...
        if (text.startsWith("/start") && text.trim() !== "/start ref") {
          ...
          if (payloadCmd) {
            ...
            if (payloadCmd.startsWith("tk_")) {
              ...
              let tokenData = null;
              let tokenDataStr = await kvRef.get(payloadCmd);
              if (tokenDataStr) {
                try { tokenData = JSON.parse(tokenDataStr); } catch(e) {}
              }

              if (!tokenData || String(tokenData.u) !== String(userId)) {
                let botUser = "Unknown_Bot";
                try {
                  const bRes = await fetch(`${TG_API}/getMe`);
                  const bData = await bRes.json();
                ...
                const kb = [[{ text: "🔗 Earn More Points", url: `https://t.me/${botUser}?start=ref` }]];
                ...
                await tgSend(TG_API, chatId, errMsg, kb);
                return new Response("OK");
              }
            ...
            if (payloadCmd.startsWith("watchlist")) {
              const filter = payloadCmd.split("_")[1] || "all";
              await showWatchlist(BOT_TOKEN, chatId, filter, env, userId, null, TG_API);
              return new Response("OK");
            }
...
async function sendSearchResults(api, botToken, chatId, userId, replyToMsgId, query, results, filterType, env, editMsgId = null, firstName = "User") {
  ...
  let botUser = "Unknown_Bot";
  try {
    const bRes = await fetch(`${api}/getMe`);
    const bData = await bRes.json();
    if (bData.ok) botUser = bData.result.username;
  } catch(e) {}
  keyboard.push([{ text: "❤️ Watchlist", url: `https://t.me/${botUser}?start=watchlist` }]);
